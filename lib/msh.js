...
function copyPayloadForward(action, queue) {
      ...
      var nextAction = queue[action.counter +1 ];  
       ...
       if (! nextAction.payload) {
           console.log('copying payload forward... for action ' + action) ;
           console.dir(action);
           if (action.transformer) {
                nextAction.payload = action.transformer(prevAction.response);    
           } else {
                nextAction.payload = prevAction.response;
           }
           
       } else {
           throw "payload already set for action "  + nextAction.counter + ", cannot forward payload in to it: " + log(nextAction) ;  
       }
