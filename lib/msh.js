...
var debug = process.env.MSH_DEBUG || true;
...
exports.init = function(callback, errCallback) {
    ...
    var httpChain = {
        ...
        delete: function(host, port, path){
            var action = {type: 'http', method: "DELETE", host: host, port: port, path: path};
            queue.push(action);
            return this;
        },  
        url: function(url, method, headers, body){
            var action = {type: 'url', method: method, url: url, headers: headers, payload: body};
            queue.push(action);
            return this;
        }, 
            ...
            for (; counter < queue.length ; counter++ ) {
                    ...
                    if (action.type == 'pipe') {    
                      ...
                      asyncTask = createPredicate(action, queue);
                       ...
                       asyncTask = createUrlRequest(action, errCallback); 
                    }
...
function createUrlRequest(action, errCallback) {
    //console.log('creating asyncTask http request for request ' + JSON.stringify(action)  ); 
    ...
    return  function(asyncCallback) {
        ...
        for (var key in action.headers) {
            var headerName = key;
            var headerValue = action.headers[key];
        ...
        if (action.payload != undefined) {
            ...
            //console.log("creating http request payload data = " + data);
            headers['Content-Type'] = 'application/json';
            headers['Content-Length'] = Buffer.byteLength(dataPayload,'utf8');
        }
        ...
        if (useProxy) {
          ...
          reqCallback = function(err, response) {
              var str = '';
        ...
        if (method == "delete") method = "del";
          ...
          var req = agent[method](action.url);
...
function log(action) {
   ...
   if (action.type == 'url') {
        console.log("%s %s %s",action.counter, action.method, action.url);   
    } else if (isGetOrDel(action)) {
        console.log("%s %s http://%s:%s%s response = %s", action.counter, action.method, action.host, action.port, action.path, truncate(action.response));     
    } else if (isPostOrPut(action)) {
         console.log("%s %s http://%s:%s%s payload = %s, response=%s", 
                     action.counter, action.method, action.host, action.port, action.path, truncate(action.payload), truncate(action.response));   
