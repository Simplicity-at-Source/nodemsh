...
exports.init = function(callback, errCallback) {
    ...
    var httpChain = {
        ...
        predicate: function(predicate) {
            var action = {type: 'predicate', predicate: predicate};
             queue.push(action);
            return this;
        },
        end: function() {
...
function enrichAction(action) {
     action.prevAction = queue[action.counter - 1];
     action.nextAction = queue[action.counter +1 ];  
...
function createPredicate(action, queue) {
    ...
    return  function(asyncCallback) {
            //console.log('running pipe task for action=' + JSON.stringify(action));
            action.response = action.predicate(action);     
            log(action);
